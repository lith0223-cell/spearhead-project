...
export async function POST(req: NextRequest) {
  ...
  try {
    const { subscription, endTime, exerciseName, routineId, exerciseIndex, cancelMessageId } = await req.json();
    ...
    const response = await qstash.publishJSON({
      ...
      delay: delaySec,
      body: { subscription, exerciseName: exerciseName ?? "운동", routineId, exerciseIndex },
    });
